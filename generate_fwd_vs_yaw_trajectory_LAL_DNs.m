function [ fr_image ] = generate_fwd_vs_yaw_trajectory_LAL_DNs( t_volts, volts_all, t_vel_all, yaw_vel_all, forward_vel_all, analysis_path, CALC_PSTH, analysis_path_type)
...
%cm = colormap(jet(575));
%cc = colormap(redbluecmap(150));
cc = colormap(jet(150));

WIN_BEFORE_STIM_ONSET = 1.0;

% Image X: [0 180], Y: [-2000 2000], 
if (CALC_PSTH == 1)
    Y_RANGE = 4000;
    X_RANGE = 180;
    image_cnts = cell( X_RANGE, Y_RANGE );
    fr_image = zeros( X_RANGE, Y_RANGE );
else
...
%             [ ~, volt_during_stim_t ] = find( (volt_t_down >= stim_start_t ) & ((volt_t_down <= stim_end_t )));        
        ...
        yaw_data   = yaw_keep;
        fwd_data   = fwd_keep;
        ephys_data = ephys_keep;
        
        yaw_all = horzcat(yaw_all, yaw_data);
        fwd_all = horzcat(fwd_all, fwd_data);
        
        for ii = 1:length(yaw_keep)
            cur_y_idx = floor( yaw_keep( ii ) ) + 2000; % range is [-2000 2000] => [0 4000]
            cur_x_idx = floor( fwd_keep( ii ) ) + 1;
            
            if(isempty(image_cnts{cur_x_idx, cur_y_idx}))
                image_cnts{cur_x_idx, cur_y_idx} = { ephys_data(ii), 1 };
            else                
                cur_img_data = image_cnts{cur_x_idx, cur_y_idx};
                new_img_data = { cur_img_data{1} + ephys_data(ii), cur_img_data{2}+1 };
                image_cnts{cur_x_idx, cur_y_idx} = new_img_data;
            end
        end
        
%         for ii = 1:length(yaw_data)
%             cur_ephys_idx = floor(ephys_data(ii)) + 1; % assume range: [-5 20]
%             if( cur_ephys_idx > size(cc,1) )
%                 ephys_color = cc(end,:);
%             else
%                 ephys_color = cc(cur_ephys_idx,:);
%             end
%             
%             plot( yaw_data(ii), fwd_data(ii), 'o', 'MarkerSize', 3, 'Color', ephys_color );
%         end
...
% histogram2(yaw_all, fwd_all, 50);

for yy = 1:Y_RANGE
    for xx = 1:X_RANGE
        
        cur_img_cnt = image_cnts{xx,yy};
        if( isempty( cur_img_cnt ) )
            fr_image(xx,yy) = 0;        
        else
            fr_image(xx,yy) = cur_img_cnt{ 1 } / cur_img_cnt{ 2 };
        end
    end
end
...
%saveas(f, [analysis_path '/fwd_vs_yaw_scatter_plot_' file_str '.fig' ]);
%saveas(f, [analysis_path '/fwd_vs_yaw_scatter_plot_' file_str '.png' ]);

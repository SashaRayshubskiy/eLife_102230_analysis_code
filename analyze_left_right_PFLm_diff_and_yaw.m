function analyze_left_right_PFLm_diff_and_yaw( imaging_data_1, PFL_FB_dF_F_per_trial, PFL_LAL_dF_F_per_trial, bdata_vel_time, bdata_vel, analysis_path, VPS, sid )
...
%     caxis([-0.5 2]);
    ...
    left_PFLm_LAL_axon_tract  = squeeze( cur_PFL_LAL(2,:));

    right_PFLm_LAL_axon_end    = squeeze( cur_PFL_LAL(3,:));
    right_PFLm_LAL_axon_tract  = squeeze( cur_PFL_LAL(4,:));
    ...
    left_PFLm_LAL_axon_tract_filt = left_PFLm_LAL_axon_tract_filt - mean( left_PFLm_LAL_axon_tract_filt );
    right_PFLm_LAL_axon_tract_filt = right_PFLm_LAL_axon_tract_filt - mean( right_PFLm_LAL_axon_tract_filt );
    ...
    % plt1 = plot( t, left_PFLm_LAL_axon_tract_filt, 'color', 'r', 'LineStyle', '--' );    
    plt1 = plot( t, left_PFLm_LAL_axon_end_filt, 'color', 'r', 'LineWidth', 2 );        
    
    %plt2 = plot( t, right_PFLm_LAL_axon_tract_filt, 'color', 'g', 'LineStyle', '--' );
    plt2 = plot( t, right_PFLm_LAL_axon_end_filt, 'g', 'LineWidth', 2 );        
    ylim([-0.8 0.8]);

    ax( 3 ) = subplot( 4, 1, 3 );
    left_right_PB_LAL_delta_end = left_PFLm_LAL_axon_end_filt - right_PFLm_LAL_axon_end_filt;
    % left_right_PB_LAL_delta_end = left_right_PB_LAL_delta_end - mean(left_right_PB_LAL_delta_end);
    ...
    hold on;
    plot( t, left_right_PB_LAL_delta_end,   'LineWidth', 2, 'color', 'b', 'DisplayName', 'axon end' );      
    %plot( t, left_right_PB_LAL_delta_tract, 'color', 'b', 'LineWidth', 2, 'DisplayName', 'axon tract' );      
    ylim([-0.8 0.8]);
    legend();

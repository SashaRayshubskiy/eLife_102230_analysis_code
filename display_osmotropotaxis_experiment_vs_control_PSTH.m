function display_osmotropotaxis_experiment_vs_control_PSTH( LAL_DN_Type, t_volts, volts_all, t_vel_all, yaw_vel_all, forward_vel_all, analysis_dir )
...
t_vel = squeeze(t_vel_all{1}{1}{1}(1,:));
...
for tt = 1:3
    ...
    for e_type = 1:2
        ...
        all_psth = [];
        for trial_idx = 1:size(all_volts,1)
            all_psth(trial_idx,:) =  calculate_psth( t, t_vel, all_volts( trial_idx, : ), ephys_SR, SPIKE_THRESHOLD, BIN_SIZE );
        end
        
        avg_psth = squeeze( mean( all_psth ) );        
        std_psth_trace = squeeze(std( all_psth, 1 ))/ sqrt(size( all_psth, 1 ) );                
               
        fh = fill([ t_vel fliplr(t_vel) ], [(avg_psth+std_psth_trace) fliplr((avg_psth-std_psth_trace)) ], cur_color_single );
        set(fh, 'EdgeColor', 'None');
        ...
        plot( t_vel, avg_psth, 'color', cur_color_avg, 'LineWidth', 2, 'LineStyle', lineStyle );
    end
...
saveas(f, [analysis_dir '/osmotropotaxis_vs_control_behavior_and_PSTH_avg.fig']);
saveas(f, [analysis_dir '/osmotropotaxis_vs_control_behavior_and_PSTH_avg.png']);

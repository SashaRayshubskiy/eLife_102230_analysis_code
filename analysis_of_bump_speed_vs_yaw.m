...
for d = 1:length(exp_dirs)
    ...
    for s = 1:length( cur_stims_to_include )
        ...
        cur_EB_bump = get_radial_weighted_avg_bump_pos_v2( cur_bump_rois );  
        ...
        min_len = min( length(EB_vel_filt), length(cur_yaw_in_interval_down) );
        ...
        if 1
            ...
            subplot(2,1,2);
            plot( t_bump_w(EB_interval), cur_EB_bump_in_interval );
            xlabel('Time (s)');
            waitforbuttonpress
        end

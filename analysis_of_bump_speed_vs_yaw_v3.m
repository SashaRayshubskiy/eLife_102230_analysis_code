function analysis_of_bump_speed_vs_yaw_v3( exp_dirs, VPS, analysis_path )
...
for d = 1:length(exp_dirs)
   ...
   while( j < length(stim_ids) )
        ...
        EB_bump_vel_align = good_bump_16( j, 2 ) + cur_start_t;
        j = j + 1;
   ...
   yyaxis right;
   hold on;
   plot( t_yaw_win, mean(yaw_win), '-', 'LineWidth', 3.0 );
   ylabel('Yaw (au/s)');
   xlabel('Time (s)');
   
   saveas( f, [analysis_path '/bump_return_vs_yaw.fig'] ); 
   saveas( f, [analysis_path '/bump_return_vs_yaw.png'] ); 

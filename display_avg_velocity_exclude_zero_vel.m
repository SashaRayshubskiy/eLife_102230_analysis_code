function [ output_args ] = display_avg_velocity_exclude_zero_vel( sid, bdata_raw, bdata_vel, bdata_vel_time, analysis_path, with_single_trials )
...
kept_both = size(kept_trials{ac.BOTH},1);
kept_left = size(kept_trials{ac.LEFT},1);
kept_right = size(kept_trials{ac.RIGHT},1);

total_trial_cnt = kept_both + kept_left + kept_right;
...
mean_both_vel_fwd = squeeze(mean(kept_trials{ac.BOTH}(:,ac.VEL_FWD,:)));
...
if( SHOW_SEM )
    ...
    sem_both_vel_fwd = squeeze(std(kept_trials{ac.BOTH}(:,ac.VEL_FWD,:),1));
end
...
saveas(f, [analysis_path '/avg_velocity_exclude_zero_vel_sid_' num2str( sid ) '.fig']);
saveas(f, [analysis_path '/avg_velocity_exclude_zero_vel_sid_' num2str( sid ) '.png']);

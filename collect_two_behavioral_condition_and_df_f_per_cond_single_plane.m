function [ btraces_per_condition, avg_df_f_per_condition_per_plane ] = collect_two_behavioral_condition_and_df_f_per_cond_single_plane( condition_trials, cdata_raw, bdata_vel, VPS, trial_exclusion_list, btrial_meta)
...
for trial_type = 1:size( condition_trials, 1 )
    ...
    for cond_ord = 1:2
        ...
        for trial_ord_idx = 1:length( cur_condition_trial_ords)
            ...
            for p = 1:PLANES                           
                cdata_in_plane = trial_cdata;
                t_size = size(cdata_in_plane,3);
                
                avg_raw_per_plane(p,:,:,1:t_size) =  squeeze(avg_raw_per_plane(p,:,:,1:t_size)) + cdata_in_plane;

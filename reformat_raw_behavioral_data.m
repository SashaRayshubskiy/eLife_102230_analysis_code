function [ bdata_time_out, bdata_vel ] = reformat_raw_behavioral_data( bdata_time_in, bdata_raw )
...
for i=1:trial_type_cnt
    ...
    [ bdata_time_out, dummy1, dummy2, dummy3 ] = get_velocity(bdata_time_in, squeeze(bdata_raw{i}(1,:,:)));
    
    bdata_vel{ i } = zeros(trial_per_type_cnt, 3, length(bdata_time_out), 'double' );
    
    for j = 1:trial_per_type_cnt    
        [ bdata_time_out, vel_fwd, vel_side, vel_yaw ] = get_velocity(bdata_time_in, squeeze(bdata_raw{i}(j, :,:)));

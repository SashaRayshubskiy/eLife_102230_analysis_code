...
% for d = 1
    ...
    for cond = 1:size( ephys_win_all,1 )
        ...
        for tr = 1 : size( cur_yaw_all, 1 )
            ...
            SHIFT_FACTOR = 3;
            for ii = 1:(length(yaw_down)-SHIFT_FACTOR)
            ...
            for ii = 1:(length(cur_yaw_t_down)-SHIFT_FACTOR)
    ...
    saveas(f,[analysis_path '/Vm_vs_yaw_spontaneous_CX_turning_FR.fig']);
    saveas(f,[analysis_path '/Vm_vs_yaw_spontaneous_CX_turning_FR.png']);
    % close(f);
end

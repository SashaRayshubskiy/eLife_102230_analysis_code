...
base_begin = 1;
base_end = floor(prestim*FPS);

ysize = size(cdata_raw{ 1 }, 2 );
xsize = size(cdata_raw{ 1 }, 3 );
...
for trial_type = 1:trial_type_cnt
    ...
    for trial = 1:cur_trial_cnt
        ...
        cur_fname = ['time_courses_in_roi_sid_' num2str(sid) '_' cur_trial_type_str '_tid_' num2str(cur_trial_id)];
        cur_tbt_filename_prefix = [ tbt_analysis_path '/' cur_fname ];        

...
datafile = [analysis_path '/directories_to_analyze.mat'];
...
if(~exist(datafile, 'file'))
    ...
    save(datafile, 't_volts', 'volts_all', 't_vel_all', 'yaw_vel_all', 'forward_vel_all', 'directories_to_analyze');
else
   dd = load(datafile);
...
generate_optimal_shift( t_volts, volts_all, t_vel_all, yaw_vel_all, forward_vel_all, analysis_path, CALC_PSTH, analysis_path_type)

%% Generate optimal shifts
generate_random_shift( t_volts, volts_all, t_vel_all, yaw_vel_all, forward_vel_all, analysis_path, CALC_PSTH, analysis_path_type)
...
CALC_PSTH = 1;
fr_image = generate_fwd_vs_yaw_trajectory_LAL_DNs(t_volts, volts_all, t_vel_all, yaw_vel_all, forward_vel_all, analysis_path, CALC_PSTH, analysis_path_type);

%% Display fr_image

DOWN_YAW = 2;
DOWN_FWD = 1;
image_cnts_reshaped = reshape(fr_image, [DOWN_FWD, size(fr_image,1)/DOWN_FWD, DOWN_YAW, size(fr_image,2)/DOWN_YAW]);
fr_image_d = squeeze(mean(squeeze(mean(image_cnts_reshaped, 1)), 2));

f = figure; 
imagesc([-2000 2000], [0 180], flipdim(fr_image_d,1));
set(gca,'YTickLabel',flipud(get(gca,'YTickLabel')));
colormap jet;
caxis([0 60]);
xlabel('Yaw (deg/s)');
ylabel('Fwd (mm/s)');
title(['Fwd bin size: ' num2str(DOWN_FWD) '   Yaw bin size: ' num2str(DOWN_YAW) ]);
set(gca, 'FontSize', 16);

saveas(f, [analysis_path '/fwd_vs_yaw_fr_colorcode_img.fig' ]);
saveas(f, [analysis_path '/fwd_vs_yaw_fr_colorcode_img.png' ]);

%%
%[xq,yq] = meshgrid(-2000:.2:2000, 0:.2:180);

% xx  = [1:size(fr_image_d,1)];
% yy  = [1:size(fr_image_d,2)];
% vq = griddata(xx,yy,fr_image_d,xq,yq, 'natural');
% 
% figure;
% mesh(xq,yq,vq);

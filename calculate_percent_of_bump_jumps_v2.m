function [ bump_jump_percent_per_fly ] = calculate_percent_of_bump_jumps_v2( basedir, cur_dirs, experiment_type_str )
...
JUMP_LOOKAHEAD_WINDOW = 0.85; % seconds

BASELINE_BUMP_BEFORE_STIM_T = 0.15; % seconds
STABLE_BUMP_BEFORE_STIM_T = 1.0; % seconds

PRE_STIM_BUMP_STABILITY_THRESHOLD = 0.5; % units of EPG wedges
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% for d = 1
    ...
    pre_stim_stability_t = find( (t_bump_w < 0) & ( t_bump_w > -1*STABLE_BUMP_BEFORE_STIM_T ));        

    pre_stim_bump_t      = find( (t_bump_w < 0) & ( t_bump_w > -1*BASELINE_BUMP_BEFORE_STIM_T ));        
...
%         subplot(2,1,1);
%         % hold on;
%         plot( t_bump_w, cur_bump_tc, 'DisplayName', [num2str(st)]);
        
        % subplot(1,1,1);
        % hold on;
        % plot( t_bump_w, cur_bump_tc_unwrapped, 'DisplayName', [num2str(st)]);
        
        [ cur_bump ] = assess_and_fix_bump_quality_v2( cur_bump_tc_unwrapped, VPS );
        ...
        if(length(cur_bump) == 0 )
            ...
            subplot(1,2,2);
            hold on;
            plot( t_bump_w, cur_bump_tc_unwrapped, 'DisplayName', ['stim: ' num2str(st)] );
            % stop_here = stop_here + 1;
        else
            
            BUMP_TC_FILTER_SAMPLE_POINTS = 20;
            % baseline_vals_uw = cur_bump_tc_unwrapped( pre_stim_bump_t );
            baseline_vals_uw = cur_bump( pre_stim_bump_t );
            baseline_non_nan_uw = baseline_vals_uw(~isnan(baseline_vals_uw));
            bump_delta_tc_uw = medfilt1( cur_bump - mean(baseline_non_nan_uw), BUMP_TC_FILTER_SAMPLE_POINTS, 'truncate' );
            
            bump_delta_tc_uw = bump_delta_tc_uw - mean( bump_delta_tc_uw( pre_stim_bump_t ) );
            
            
            pre_stim_bump_tc_uw = bump_delta_tc_uw( pre_stim_stability_t );
            
            
            %         plot( t_bump_w, bump_delta_tc_uw, 'DisplayName', [num2str(st)], 'color', 'g');
            %         waitforbuttonpress;
            %         cla();
            
%             if ( std( pre_stim_bump_tc_uw ) <= PRE_STIM_BUMP_STABILITY_THRESHOLD  )
                ...
                subplot(1,2,1);
                hold on;
                plot( t_bump_w, bump_delta_tc_uw, 'DisplayName', ['stim: ' num2str(st)] );
%             else
%                 subplot(1,2,2);
%                 hold on;
%                 plot( t_bump_w, cur_bump_tc_unwrapped, 'DisplayName', ['stim: ' num2str(st)] );
%             end
    ...
    title(['Fly: ' num2str(d) ' total passed stims: ' num2str( total_passed_stims ) ' jump cnt: ' num2str(jump_cnt) ' percent: ' num2str( cur_percentage_jump ) ]);
    
    saveas( f, [ analysis_path '/jump_statistics.fig' ] );
    saveas( f, [ analysis_path '/jump_statistics.png' ] );    
end
end

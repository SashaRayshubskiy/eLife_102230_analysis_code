function display_CX_summary_bar_plots_all_flies_use_mean( bump_conditions, bump_conditions_str,  bump_pos_win_all, bump_win_all, ...
...
BUMP_VEL_RANGE_MAX =  0.150;

YAW_VEL_RANGE_MIN = BUMP_VEL_RANGE_MIN;
YAW_VEL_RANGE_MAX = BUMP_VEL_RANGE_MAX;

EPHYS_RANGE_MIN = -0.65;
EPHYS_RANGE_MAX =  0.0;
...
for cond = 1:size( bump_win_all, 1 )
    ...
    for d = 1:size( bump_win_all, 2 )
        ...
        yaw_vel_param_in_range{cond}(end+1) = mean( avg_yaw_for_cur_fly( yaw_vel_in_range_idx ));
        ...
        delta_avg_Vm = avg_Vm_for_cur_fly - mean( avg_Vm_for_cur_fly( ephys_baseline_in_range_idx ));
        ...
        Vm_param_in_range{cond}(end+1) = mean( delta_avg_Vm( ephys_in_range_idx ) );
        ...
        delta_avg_PSTH = avg_PSTH_for_cur_fly - mean( avg_PSTH_for_cur_fly( yaw_vel_baseline_in_range_idx ));
        
        % FR_param_in_range{cond}(end+1) = mean( avg_PSTH_for_cur_fly( yaw_vel_in_range_idx )) - mean( avg_PSTH_for_cur_fly( yaw_vel_baseline_in_range_idx ));
        FR_param_in_range{cond}(end+1) = mean( delta_avg_PSTH( yaw_vel_in_range_idx ));
                
%         subplot(2,1,1);
%         hold on;
%         plot( Vm_t, delta_avg_Vm );
% 
%         subplot(2,1,2);
%         hold on;
%         plot( yaw_t, delta_avg_PSTH );
    end
...
saveas(f, [ CX_summary_path '/' experiment_type_str '_bump_conditions_all_bar_plots_flies_' num2str(YAW_VEL_RANGE_MIN) '.fig' ]);
saveas(f, [ CX_summary_path '/' experiment_type_str '_bump_conditions_all_bar_plots_flies_' num2str(YAW_VEL_RANGE_MIN) '.png' ]);

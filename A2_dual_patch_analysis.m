...
subplot(1,3,1);
plot(A2_L_psth_down(1:end-3), yaw_all_down(4:end), 'o', 'MarkerSize', 3);
...
subplot(1,3,2);
plot(A2_R_psth_down(1:end-3), yaw_all_down(4:end), 'o', 'MarkerSize', 3);
...
subplot(1,3,3);
plot(A2_L_R_diff_psth_down(1:end-3), yaw_all_down(4:end), 'o', 'MarkerSize', 3);
xlabel('A2 L-R PSTH diff (spikes/s)');
ylabel('Yaw (deg/s)');
title('Left-Right A2 vs. yaw');
grid on;

saveas(f,[analysis_path '/A2_right_left_PSTH_diff_vs_yaw_scatter.fig']);
saveas(f,[analysis_path '/A2_right_left_PSTH_diff_vs_yaw_scatter.png']);
